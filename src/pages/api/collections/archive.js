...
import ScrollWrapper from "@/components/ScrollWrapper"; // adjust if needed
...
export default function SongArchivePage() {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/songs/archive`)
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-black text-yellow-100 px-4 sm:px-6 py-12 font-ticket">
      <Head>
                        ...
                        <div className="grid grid-cols-2 gap-1 sm:grid-cols-3 sm:gap-3">
                          {group.map(({ name, slug }) => (
                            <Link
                              key={slug}
                              href={`/songs/${slug}`}
                              className="block px-2 py-1 sm:px-2.5 sm:py-1.5 rounded border border-yellow-800 bg-yellow-900/30 hover:bg-yellow-700/50 transition-all text-center text-sm sm:text-base sm:text-[1.05rem] leading-snug box-border"
                            >
                              {name}
                            </Link>
                          ))}
