...
function PlaylistPickerModal({ playlists, entryId, onClose, onAdd }) {
  ...
  return (
    <div className="space-y-4">
          ...
          {playlists.map((p) => (
            <li key={p.id} className="flex items-center justify-between">
              ...
              <div className="ml-2 relative">
                <button
                  onClick={() => setOptionsVisibleId(optionsVisibleId === p.id ? null : p.id)}
                  className="text-white/30 hover:text-white"
                  title="Options"
                >⚙️</button>
                {optionsVisibleId === p.id && (
      ...
      <h3 className="text-md font-semibold text-white mt-4">Or create a new playlist</h3>
          ...
          return (
            ...
            >
              <div className="flex-1 overflow-hidden">
                <span className="block text-white font-medium truncate">{entry.song}</span>
                {entry.isEncore && (
              ...
              <div className="flex items-center gap-3 flex-shrink-0">
                ...
                <button
                  onClick={() => setModalEntryId(entry.id)}
